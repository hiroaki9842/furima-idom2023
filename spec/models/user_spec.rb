  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できないとき' do
      ...
      it 'birthday_yyyy_mm_ddが空の場合は登録できない' do
        ...
        it 'nicknameとemail、passwordとpassword_confirmation、kanji_fist_name、kanji_last_name、kana_first_name、kana_last_name、bithday_yyyy_mm_ddが存在すれば登録できる' do
          expect(@user).to be_valid
        end
      end
      context '新規登録できないとき' do
        it 'nicknameが空では登録できない' do
          @user.nick_name = '' #カラム名の変更に伴いメソッド名の変更
          @user.valid?
        ...
        it 'パスワードは、英字のみ又は数字のみでは登録できない' do
          ...
          @user.valid?
          except(@user.errors.full_messages).to include('Passwords cannot be registered with only letters or numbers.')#エラー文言追加7/31
        end
        it 'passwordとpassword_confirmationが不一致では登録できない' do
          @user.password = '111aaa'
          @user.password_confirmation = '222aaa'
        ...
        it 'kanji_fist_nameが全角（漢字・ひらがな・カタカナ）でない場合は登録できない' do
          ...
          @user.valid?
          except(@user.errors.full_messages).to include('First name is not full-width (kanji, hiragana, katakana), it cannot be registered.')#エラー文言追加7/31
        end
        it 'kanji_last_nameが全角（漢字・ひらがな・カタカナ）でない場合は登録できない' do
          @user.kanji_last_name = 'ishihara'
          @user.valid?
          except(@user.errors.full_messages).to include('Last name is not full-width (kanji, hiragana, katakana), it cannot be registered.')#エラー文言追加7/31
        end
        it 'kana_first_name又はkana_last_nameが空では登録できない' do
          @user.kana_first_name = ''
          @user.valid?
        ...
        it 'kana_first_nameが全角（カタカナ）でない場合は登録できない' do
          @user.kana_first_name = 'さとみ'
          @user.valid?
          except(@user.errors.full_messages).to include('First name is not full-width (katakana), it cannot be registered.')#エラー文言追加7/31
        end
        it 'kana_last_nameが全角（カタカナ）でない場合は登録できない' do
          @user.kana_last_name = '石原'
          @user.valid?
          except(@user.errors.full_messages).to include('Last name is not full-width (katakana), it cannot be registered.')#エラー文言追加7/31
        end
        it 'bithday_yyyy_mm_ddが空の場合は登録できない' do
          @user.bithday_yyyy_mm_dd = ''
          @user.valid?
